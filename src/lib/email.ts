...
function getFromAddress(): string {
  ...
  return addr.includes("<") ? addr : `AlmiTOEFL <${addr}>`;
}
...
// AlmiTOEFL palette — mirrors src/lib/brand.ts.
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;background-color:${COLOR_PAPER};border-radius:8px;padding:32px;">
            <tr>
              <td style="font-size:16px;line-height:1.6;color:${COLOR_TEXT};">
                ${bodyInner}
                <p style="margin:24px 0 0 0;color:${COLOR_TEXT_MUTED};font-size:14px;">— AlmiTOEFL<br /><a href="${SITE_URL}" style="color:${COLOR_TEXT_MUTED};">${SITE_URL}</a></p>
              </td>
            </tr>
          </table>
        </td>
...
function renderPasswordResetHtml(resetUrl: string): string {
  ...
  return shell(`
    ...
    <p style="margin:0 0 16px 0;">Someone (hopefully you) requested a password reset for your AlmiTOEFL account.</p>
...
Someone (hopefully you) requested a password reset for your AlmiTOEFL account.
...
export async function sendPasswordResetEmail(input: {
  ...
  const result = await client.emails.send({
    from: getFromAddress(),
    to: input.to,
    subject: "Reset your AlmiTOEFL password",
...
function renderEmailVerificationHtml(verifyUrl: string): string {
  ...
  return shell(`
    <p style="margin:0 0 16px 0;">Assalam o alaikum,</p>
    <p style="margin:0 0 16px 0;">Welcome to AlmiTOEFL. One more step — please confirm this email address so we know it's really you.</p>
    ${ctaButton(safe, "Verify my email")}
    <p style="margin:0 0 8px 0;font-size:14px;color:${COLOR_TEXT_MUTED};">Or copy and paste this link:</p>
    <p style="margin:0 0 16px 0;font-size:14px;color:${COLOR_TEXT_MUTED};word-break:break-all;"><a href="${safe}" style="color:${COLOR_TEXT_MUTED};">${safe}</a></p>
    <p style="margin:0 0 16px 0;">This link expires in 24 hours.</p>
    <p style="margin:0;">If you didn't sign up for AlmiTOEFL, you can safely ignore this email.</p>
  `);
}
...
Welcome to AlmiTOEFL. One more step — please confirm this email address so we know it's really you.
...
If you didn't sign up for AlmiTOEFL, you can safely ignore this email.
...
export async function sendEmailVerification(input: {
  ...
  const result = await client.emails.send({
    from: getFromAddress(),
    to: input.to,
    subject: "Verify your AlmiTOEFL email",
