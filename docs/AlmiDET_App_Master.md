# AlmiDET — App Master / Copy Doc

*The single source of truth for the AlmiDET practice app: verified DET format, scoring model, v1 scope, honesty + IP doctrine, and the user-facing copy. Engine forked from AlmiTOEFL; item-bank model ported from AlmiPTE. Prepared 2026-06-24.*
...
Sources: englishtest.duolingo.com + Duolingo blog/handbook + corroborating third-party prep sites (goarno, abroadcube, leverageedu, scholastictestmasters). Corrections vs. the original roadmap assumptions:
...
- **Overall score**: DET's published illustrations compute overall as the average of the four skill scores; the four *integrated* subscores are each the average of two skills. DET's real overall is an adaptive/IRT estimate, not a public arithmetic formula. **AlmiDET does not compute or display an overall** — see §3.
...
- **Subdomain:** `almidet.almiworld.com`
- **Repo / Vercel project:** `almi-det` (owner `smnasiruz016-blip`, Vercel org `almiworld`), own Neon DB
- **Session cookie:** `almi_det_session` (do NOT inherit `almi_prep_session`)
- **Stripe metadata:** `product: "almi-det"`
...
## 2. The DET format AlmiDET must match (authoritative)
...
## 3. AlmiDET scoring model (honesty-first)

**Non-negotiable: AlmiDET never fabricates a single 10–160 overall.** DET's overall is a proprietary adaptive estimate we cannot and will not reproduce. Instead:
...
- **100% original practice material — "never copied from Duolingo."** All stimuli (words, audio scripts, photos/prompts) are written from scratch. Codified in (a) every AI evaluation system prompt, and (b) user-facing copy. (Mirrors AlmiPTE "never copied from Pearson" / AlmiTOEFL "never copied from ETS".)
...
> **Practise the Duolingo English Test with honest AI feedback.**
...
> This is a practice estimate to guide your prep — not an official Duolingo English Test result. Your real score comes only from an official test.

**"Never copied" line (practice hub + each picker):**
> Every task here is written from scratch by AlmiDET. We never copy or reproduce Duolingo's test material.
